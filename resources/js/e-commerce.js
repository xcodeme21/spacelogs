document.addEventListener('DOMContentLoaded', function () {
  ...
  const showModal = async (tableName, title, elementId, intervalDuration = 60000) => {
    ...
    try {
      const response = await fetch(
        `/hit-count?transaction_title=${encodeURIComponent(title)}&table_name=${encodeURIComponent(tableName)}`
      )
      ...
      fetchInterval = setInterval(
        () => showModal(tableName, title, elementId, intervalDuration),
        intervalDuration
      )
    } catch (error) {
  ...
  const api = {
    url_oaa_stockquery: '/api/e-commerce/oaa-stockquery',
    url_opp_item: '/api/e-commerce/opp-item',
    url_opp_checkout: '/api/e-commerce/opp-checkout',
  }
